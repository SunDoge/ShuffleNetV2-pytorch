...
parser.add_argument('--decay', '-d', type=float,
                    default=4e-5, help='Weight decay (L2 penalty).')
parser.add_argument('--gamma', type=float, default=0.1,
                    help='LR is multiplied by gamma at scheduled epochs.')
...
parser.add_argument('--mode', default='triangular2',
                    help='CLR mode. One of {triangular, triangular2, exp_range}')
...
parser.add_argument('-e', '--evaluate', dest='evaluate',
                    action='store_true', help='Just evaluate model')
parser.add_argument('--save', '-s', type=str, default='',
                    help='Folder to save checkpoints.')
parser.add_argument('--results_dir', metavar='RESULTS_DIR',
                    default='./results', help='Directory to store results')
parser.add_argument('--resume', default='', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--start-epoch', default=0, type=int,
                    metavar='N', help='manual epoch number (useful on restarts)')
parser.add_argument('--log-interval', type=int, default=100,
                    metavar='N', help='Number of batches between log messages')
parser.add_argument('--seed', type=int, default=None,
                    metavar='S', help='random seed (default: random)')

# Architecture
parser.add_argument('--scaling', type=float, default=1,
                    metavar='SC', help='Scaling of ShuffleNet (default x1).')
parser.add_argument('--input-size', type=int, default=224,
                    metavar='I', help='Input size of ShuffleNet.')
...
parser.add_argument('--residual', dest='residual',
                    action='store_true', help='Just residuals')
parser.add_argument('--groups', type=int, default=2,
                    help='Groups per shuffle.')

# https://arxiv.org/abs/1807.11164
#g, SE, scale
claimed_acc_top1 = {False: {0.5: 0.397, 1.: 0.306,
                            1.5: 0.274, 2.: 0.251}, True: {2.: 0.346}}
...
def main():
    ...
    model = ShuffleNetV2(scale=args.scaling, c_tag=args.c_tag,
                         SE=args.SE, residual=args.residual, groups=args.groups)
    ...
    print('FLOPs: {}'.format(
        flops_benchmark.count_flops(ShuffleNetV2,
                                    args.batch_size // len(
                                        args.gpus) if args.gpus is not None else args.batch_size,
    ...
    if args.evaluate:
        loss, top1, top5 = test(
            model, val_loader, criterion, device, dtype)  # TODO
        return
    ...
    if args.SE in claimed_acc_top1:
        if args.scaling in claimed_acc_top1[args.SE]:
            ...
            csv_logger.write_text(
                'Claimed accuracy is {:.2f}% top-1'.format(claimed_acc1 * 100.))
...
def train_network(start_epoch, epochs, scheduler, model, train_loader, val_loader, optimizer, criterion, device, dtype,
    ...
    for epoch in trange(start_epoch, epochs + 1):
        ...
        csv_logger.plot_progress(
            claimed_acc1=claimed_acc1, claimed_acc5=claimed_acc5)
    ...
    csv_logger.write_text(
        'Best accuracy is {:.2f}% top-1'.format(best_test * 100.))
